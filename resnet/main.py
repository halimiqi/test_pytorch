...
from resnet import torch_model as models
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                     std=[0.229, 0.224, 0.225])   # here should be modified
    ...
    train_dataset = datasets.ImageFolder(
        ...
        transforms.Compose([
            ...
            transforms.ToTensor(),
            normalize, # compose these transforms in sequence
        ])
    )
    ...
    val_loader = torch.utils.data.DataLoader(
        ...
        num_workers=args.workers, pin_memory=True)  # the data_loader receive a dataset object and have the functions to shuffle and batch the dataset.
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        loss = criterion(output, target) # get the gradient
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad(): # in the validate stage, don't calculate the gradient
        end = time.time()
...
def save_checkpoint(state, is_best, filename = 'checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, 'model_best.pth.tar')  # copy the file into best model path
